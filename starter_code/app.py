...
def venues():
  ...
  # for city in cities:
  #   ids, names = [r[0] for r in Venue.query.filter_by(city=city).values('id')], [r[0] for r in Venue.query.filter_by(city=city).values('name')]
  #   data.append({'city': city, 'state': db.session.query(Venue).filter_by(city=city).first().state, 'venues': [{'id': idi, 'name': name} for idi, name in zip(ids, names)]})
  # data=[{
  #   "city": "San Francisco",
  #   "state": "CA",
  #   "venues": [{
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    # artist_to_edit.seeking_venue = True if 'seeking_venue' in request.form else False
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    # venue_to_edit.seeking_venue = True if 'seeking_venue' in request.form else False
    ...
    db.session.close()
  if error:
    flash('An error occurred. venue_to_edit could not be changed.')
  if not error:
    flash('venue_to_edit was successfully updated!')
